...
class PetShopAdminForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if kullanici_adi and User.objects.filter(username=kullanici_adi).exists():
            ...
            if existing_user.is_superuser:
                raise forms.ValidationError(
                    "Admin kullanıcısına petshop profili oluşturulamaz! "
                    "Admin kullanıcıları sistem yöneticisidir, petshop olamaz."
                )
        ...
        if telefon:
            ...
            if Veteriner.objects.filter(telefon=telefon_clean).exists():
...
class PetShopAdmin(admin.ModelAdmin):
    form = PetShopAdminForm
    list_display = (
        "ad", "il_display", "ilce_display", "telefon",
        "odeme_modeli", "magaza_tipi", "tahsis_sayisi", "satis_sayisi",
        "kalan_envanter_goster", "aktif",
    )
    list_filter = ("aktif", "odeme_modeli", "il", "ilce", "magaza_tipi")
    search_fields = ("ad", "telefon", "email", "il__ad", "ilce__ad")
    ordering = ("-olusturulma",)
    
    class Media:
        js = ('admin/js/petshop_admin.js',)
    ...
    fieldsets = (
        ...
        ("Mağaza Bilgileri", {
            "fields": ("magaza_tipi", "magaza_buyuklugu", "calisan_sayisi", "kurulus_yili")
        }),
        ("Hizmetler", {
            "fields": ("pet_kuafor", "pet_hotel", "pet_taksi", "pet_egitim", "pet_bakim")
        }),
        ("Sistem Bilgileri", {
        ...
        if db_field.name == "ilce":
            ...
            if request.resolver_match.kwargs.get('object_id'):
                try:
                    ...
                    obj = PetShop.objects.get(pk=obj_id)
                    if obj.il:
                        il_id = obj.il.id
                except (PetShop.DoesNotExist, ValueError, KeyError):
                    pass
    ...
    def save_model(self, request, obj, form, change):
        ...
        if kullanici_adi and sifre and not obj.kullanici:
            ...
            success_msg = format_html(
                ...
                '<h4 style="margin-top: 0;">✅ PetShop Kullanıcı Hesabı Oluşturuldu</h4>'
                '{}'
                '<p><strong>Bayiye vereceğiniz bilgiler:</strong></p>'
                '<table style="border-collapse: collapse; width: 100%;">'
                '<tr><td style="padding: 5px; border: 1px solid #ddd; background: #f8f9fa;"><strong>PetShop Adı:</strong></td>'
                ...
                '🔒 <strong>İlk Giriş Şifre Değiştirme:</strong> PetShop ilk giriş yaptığında şifresini değiştirmesi ZORUNLUDUR. '
            ...
            if obj.email:
                try:
                    ...
                    subject = f"PetSafe Hub - PetShop Hesabınız Oluşturuldu"
                    message = f"""Merhaba {obj.ad},

PetShop hesabınız başarıyla oluşturulmuştur.
...
class SiparisIstemiAdmin(admin.ModelAdmin):
    # Liste görünümü
    list_display = (
        "petshop", "talep_edilen_adet", "talep_tarihi",
    ...
    search_fields = (
        "petshop__ad", "petshop__il__ad", "petshop__ilce__ad",
    ...
    fieldsets = (
        ("Sipariş", {
            "fields": ("petshop", "talep_edilen_adet", "talep_tarihi", "onaylandi", "onay_tarihi")
        }),
        ("Gönderim Adresi", {
